...
public:
	...
	static bool FindAndSendCommandCallback(const char* szCommand, int iSlot, const char* szContent, bool bConsole) {
		...
		if(bConsole)
		{
			...
			{
				auto it = item.second.find(std::string(szCommand));
				if (it != item.second.end() && it->second &&
					it->second(iSlot, fullCommand.c_str()))
				{
					bFound = true;
				}
			}
		...
		else
		{
			...
			{
				auto it = item.second.find(std::string(szCommand));
				if(it != item.second.end() && it->second && it->second(iSlot, szContent))
				{
					bFound = true;
				}
			}
	...
	void ClearAllHooks(SourceMM::PluginId id) override {
		ChatHookPre.erase(id);
		ChatHookPost.erase(id);

		StartupHook.erase(id);
		MapEndHooks.erase(id);
		MapStartHooks.erase(id);
		GetGameRules.erase(id);

		HookEvents.erase(id);

		OnTakeDamageHook.erase(id);
		OnTakeDamageHookPre.erase(id);

		OnHearingClientHook.erase(id);

		ConsoleCommands.erase(id);
		ChatCommands.erase(id);
...
public:
	...
	uint64 GetUnauthenticatedSteamId64() { return m_UnauthenticatedSteamID ? m_UnauthenticatedSteamID->ConvertToUint64() : 0; }
